...
export function FloatingActionButtons({
  ...
  // Don't show chat button if there's no active room
  const shouldShowChat = showChat && activeRoom;

  // If no buttons should be shown, return null
  if (!showMyLocation && !showDirections && !shouldShowChat) {
    return null;
  }
      ...
      {shouldShowChat && (
        <TouchableOpacity
          style={[
            styles.fab,
            styles.chatButton,
            isChatOpen && styles.chatButtonActive
          ]}
          onPress={onChatPress}
          activeOpacity={0.7}
        >
